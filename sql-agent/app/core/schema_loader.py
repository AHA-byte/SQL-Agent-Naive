...
def _intent_tables_from_message(user_message: str) -> set[str]:
    ...
    if "reminder" in q:
        intent_tables.update({"t_reminders", "t_jobs", "admin.QBCC-Policies"})
    ...
    if "claim" in q or "policy" in q or "premium" in q:
        intent_tables.update({"In4mo.ClaimInformation", "admin.QBCC-Policies", "In4mo.CostControl"})
    if "inspection" in q or "property" in q or "damage" in q:
        intent_tables.update({"In4mo.ClaimInformation", "In4mo.WorkPlan", "In4mo.ProjectPlan"})
    if "qbcc" in q:
        intent_tables.update({"admin.QBCC-Policies", "t_jobs"})
    if "estimate" in q:
...
def _compute_schema_limit(user_message: str, base_limit: int) -> int:
    ...
    if mention_count >= 3:
        return max(6, int(base_limit * 0.5))
    if len(tokens) < 6:
        return min(30, int(base_limit * 1.5))
    return base_limit
